...
Not yet re-explained why `all.json`'s effective rank (57.6) is so much higher than `font_vit_embeddings.json`'s (9.5) now that the contrastive-finetuning explanation is known wrong — worth settling before leaning on it further. On the diffusion model itself: open questions left where the user paused mid-investigation are whether to push `--pcaDim` higher, train longer, or change the conditioning mechanism (currently a weak concat-into-MLP; something like cross-attention or FiLM conditioning might use the text signal harder) — versus treating the current result as a sufficient modality-gap characterization and moving on. `embeddings/allText.json` (the actual `ViTEmbedder` contrastive output, per the provenance correction) has not been run through `effective_dimension.py`/the diffusion pipeline at all yet — doing so would properly test the original "does contrastive training change this" hypothesis this investigation accidentally answered for the wrong file.
